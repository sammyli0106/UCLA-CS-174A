...
window.Movement_Controls = window.classes.Movement_Controls = class Movement_Controls extends Scene_Component {
    ...
    first_person_flyaround(radians_per_frame, meters_per_frame, leeway=70) {
		//const DIST_FROM_OBJ = 25;
		const RT_INCR = Math.PI/180;
        ...
        if (this.roll != 0) {
			// if (this.context.globals.graphics_state.allow_cam_turn) {
				// do_operation(Mat4.translation(Vec.of(0, 0, DIST_FROM_OBJ)));
				// do_operation(Mat4.rotation(-sign * RT_INCR, Vec.of(0, this.roll, 0)));
				// do_operation(Mat4.translation(Vec.of(0, 0, -DIST_FROM_OBJ)));
			// }
			this.context.globals.graphics_state.shootAngle += RT_INCR * this.roll;
		}
